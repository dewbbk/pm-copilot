...
|------|------|------------|
| id, stage, path, problem | Working | Ядро, нужно каждый ход |
| goals[] | Working | Все цели с вложенными epics[].tasks[] (при compaction — completed/archived → archive) |
| goals[].epics[] | Working | Эпики активной цели |
...
#### goals[].epics[]
- **Условие**: Эпик имеет status: completed
- **Действие**: Переместить completed эпики и их таски в archive (внутри archive.goals[])
- **Оставить**: Только planned/active эпики в Working Memory
...
2. **Learning → Goal Bridge**: При завершении post-launch — Copilot переносит уроки в контекст для формулирования новой цели: «В прошлый раз [pattern]. Учесть при формулировке новой цели?»
...
4. **Счётчик циклов**: `learning_cycle_count` инкрементируется при каждом завершённом цикле (goal → ... → post-launch). При >=3 — «зрелый продукт»
...
2. **Bridge срабатывает автоматически**: При переходе post-launch → goal — Copilot показывает топ-3 урока из последней Learning Card и спрашивает, какие учесть
...
1. **Обновляют только workflow-скиллы**: hypothesis, goal, epic, task, comms, post-launch. Остальные скиллы (domain, onboarding, thinking-in-bets, generation) не обновляют last_context — они вспомогательные.
...
- Поле `learning_cycle_count` инкрементируется при завершении цикла goal→...→post-launch (v4.15)
...
- stage = goal + KR зафиксированы → «Перейти к проработке эпика?» (→ epic)
- stage = epic + таски собраны → «Выбрать таску для проработки?» (→ task)
...
- stage = post-launch + оценка завершена → «Сформулировать новую цель?» (→ goal)
